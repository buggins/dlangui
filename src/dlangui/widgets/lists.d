...
class ListWidget : WidgetGroup, OnScrollHandler, OnAdapterChangeHandler
{
    ...
    protected void updateSelectedItemFocus()
    {
        ...
        {
            if ((_adapter.itemState(_selectedItemIndex) & State.Focused) != (
                    overrideStateForItem & State.Focused))
            {
    ...
    override void measure(int parentWidth, int parentHeight)
    {
        ...
        int sbsize = _orientation == Orientation.Vertical ? _scrollbar.measuredWidth
            : _scrollbar.measuredHeight;
        // measure children
        Point sz;
        _sbsz.destroy();
        for (int i = 0; i < itemCount; i++)
    ...
    override bool onMouseEvent(MouseEvent event)
    {
        ...
        {
            if (_scrollbar)
                _scrollbar.sendScrollEvent(event.wheelDelta > 0 ? ScrollAction.LineUp
                        : ScrollAction.LineDown);
            return true;
        }
        if (event.action == MouseAction.ButtonDown && (event.flags & (MouseFlag.LButton || MouseFlag
                .RButton)))
            setFocus();
            ...
            if (itemrc.isPointInside(Point(event.x, event.y)))
            {
                if (_adapter && _adapter.wantMouseEvents)
                {
                    ...
                    if (itemWidget)
                    {
                        ...
                        if (event.action == MouseAction.Move && event.noModifiers && itemWidget
                            .hasTooltip)
                        {
                ...
                {
                    if ((_clickOnButtonDown && event.action == MouseAction.ButtonDown) || (!_clickOnButtonDown && event
    ...
    override bool onKeyEvent(KeyEvent event)
    {
        ...
        if (event.action == KeyAction.Text)
        {
            ...
            else
            {
                ...
                if (timePassed > 0.5)
                    _searchString = ""d; // TODO_GRIM: make everything look-alike
            }
